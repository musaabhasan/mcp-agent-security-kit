...
| TS-014 | Cloud metadata service exposure through network tools | A browser, fetch, request, or HTTP-capable MCP server can reach cloud metadata endpoints or has wildcard egress, allowing prompt injection or retrieved instructions to probe instance credentials. | Deny cloud metadata endpoints, use explicit egress allowlists, run agents without ambient cloud credentials, and require review for network scope changes. | MCP-024 finding, egress policy, network trace, credential broker log. |
